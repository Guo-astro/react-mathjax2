...
export const MathJaxContext: FC<MathJaxContextProps> = ({
  config,
  version = 3,
  ...
  if (
    typeof previousContext?.version !== "undefined" &&
    previousContext?.version !== version
  )
    throw Error(
        ...
        if (typeof window !== "undefined") {
          ...
          v2Promise.catch((e) => {
            if (onError) onError(e);
              ...
              throw Error(
                `Failed to download MathJax version 2 from '${usedSrc}' due to: ${JSON.stringify(
                  e
                )}`
              );
          });
        ...
        if (typeof window !== "undefined") {
          ...
          v3Promise.catch((e) => {
            if (onError) onError(e);
              ...
              throw Error(
                `Failed to download MathJax version 3 from '${usedSrc}' due to: ${e}`
              );
          });
        } else {
          // for server side rendering
          v3Promise = Promise.reject();
          v3Promise.catch(() => undefined);
        }
      }
    }
    mjContext.current = {
